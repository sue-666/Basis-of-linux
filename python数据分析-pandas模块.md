...
## 查看 行 或 列： 通过 .loc['day1'] 访问特定行    
#loc 属性不仅支持“索引”，还支持“切片”，需要写成 loc[start:end] **包含start**   
传入一个 标签列表 可获取 loc 中不连续的行。例如 df.loc[['S01', 'S03', 'S05']]   
...
### 查看列：  print(df[['可乐', '炸鸡']])     #中括号加列名，布尔索引(比较或False或True)  
